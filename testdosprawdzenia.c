...
int main() {
    ...
    uint16_t value;
    uint16_t inner_delay=0;
    uint16_t i=0;
    
    while (1) {
    // Define static variables to keep state between loop iterations
        AD1CON1bits.SAMP = 1;
        for (volatile unsigned int b=0;b<65000;b++)
        AD1CON1bits.SAMP = 0;
        ...
        LATA = i;
        if (value < 200)       
            for (inner_delay = 0; inner_delay < 100; inner_delay++);
        else if (value < 400) {     
            for (inner_delay = 0; inner_delay < 60000; inner_delay++);
        }
        else if (value < 600)  {     
            for (inner_delay = 0; inner_delay < 100; inner_delay++);
        }
            ...
            for (inner_delay = 0; inner_delay < 60000; inner_delay++);
            for (inner_delay = 60000; inner_delay > 0; inner_delay--);
            for (inner_delay = 0; inner_delay < 60000; inner_delay++);
            for (inner_delay = 60000; inner_delay > 0; inner_delay--);
        }
        else {     
